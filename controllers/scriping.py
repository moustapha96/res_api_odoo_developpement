...
#     #                             image_base64_list = [self.get_image_base64(img['src']) for img in images]
...
#     #                                 'list_price': produit.get('price', 0.0),
#     #                                 'standard_price': produit.get('cost', 0.0),
#     #                                 'en_promo': produit.get('en_promo', False),
#     #                                 'is_preorder': produit.get('is_preorder', False),
#     #                                 'preorder_price': produit.get('preorder_price', 0.0),
#     #                                 'promo_price': produit.get('promo_price', 0.0),
#     #                                 'is_creditorder': produit.get('is_creditorder', False),
#     #                                 'creditorder_price': produit.get('creditorder_price', 0.0),
#     #                                 'rang': produit.get('rang', 0),
#     #                                 'categ_id': self.get_category_id(produit.get('categories', [])),
#     #                                 'product_tag_ids': [(6, 0, self.get_tag_ids(produit.get('tags', [])))]
#     #                             }
                                
#     #                             if image_base64_list:

#     #                                 product_vals['image_1'] = image_base64_list[1] if len(image_base64_list) > 1 else False
#     #                                 product_vals['image_2'] = image_base64_list[2] if len(image_base64_list) > 2 else False
#     #                                 product_vals['image_3'] = image_base64_list[3] if len(image_base64_list) > 3 else False
#     #                                 product_vals['image_4'] = image_base64_list[4] if len(image_base64_list) > 4 else False

#     #                             # N'ajouter l'image que si elle est valide
#     #                             if image_base64:
...
#     #                         except Exception as e:
#     #                             _logger.error(f"Erreur lors de la création du produit {produit['title']}: {str(e)}", exc_info=True)
...
#                                     'creditorder_price': produit.get('creditorder_price', 0.0),
#                                     'rang': produit.get('rang', 0),
#                                     'categ_id': categ_id,  # Utiliser la catégorie récupérée
#                                     'product_tag_ids': [(6, 0, self.get_tag_ids(produit.get('tags', [])))]
#                                 }
...
#     def get_image_base645(self, image_url):
...
#                 _logger.info(f"Image valide téléchargée: {image_url}")
