...
class SmsSender(models.AbstractModel):
    ...
    @api.model
    def send_order_details_sms(self, order_id):
        """
        Méthode pour envoyer les détails de la commande par SMS
        :param order_id: ID de la commande
        :return: dict avec status (success/error) et message
        """
        order = self.env['sale.order'].browse(order_id)
        partner = order.partner_id
        recipient = partner.mobile or partner.phone
        ...
        message += f"Merci pour votre commande #{order.name}.\n"
        message += f"Date de commande: {order.date_order}\n"
        message += f"Montant total: {order.amount_total}\n\n"
        message += "Détails de la commande:\n"

        for line in order.order_line:
            message += f"- {line.product_id.name}: {line.product_uom_qty} x {line.price_unit}\n"
